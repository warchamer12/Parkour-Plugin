...
public class ParkourConfigEasy {

    static File cfile;
    static FileConfiguration config;
    static File folder = new File(Parkour.getPlugin().getDataFolder(), "easy" + File.separator);
    static File df = Parkour.getPlugin().getDataFolder();

    public static void create() {
        if (!folder.exists()) folder.mkdir();
        cfile = new File(df, "easy" + File.separator + ParkourObject.getEasy() + ".yml");
    ...
    public static File getName(String name) {
        return cfile = new File(df, "easy" + File.separator + name + ".yml");
    }
